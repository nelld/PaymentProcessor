...
function toggleAddPaymentSection() {
    ...
    if (isActive) {
        ...
        // Show form for pre-selected option (Credit Card by default)
        const selectedOption = document.querySelector('#addPaymentOptionsSaved .add-payment-option.selected');
        if (selectedOption) {
            // Check which option is selected and show appropriate form
            const ariaLabel = selectedOption.getAttribute('aria-label');
            if (ariaLabel.includes('credit') || ariaLabel.includes('Credit')) {
                toggleNewCardForm('credit', true);
            } else if (ariaLabel.includes('debit') || ariaLabel.includes('Debit')) {
                toggleNewCardForm('debit', true);
            } else if (ariaLabel.includes('bank') || ariaLabel.includes('Bank')) {
                toggleNewBankForm(true);
            }
        }
    } else {
        // When collapsed
        ...
        const cardSection = document.getElementById('newCardSection');
        const bankSection = document.getElementById('newBankSection');
        if (cardSection) {
...
// Scroll payment options horizontally (no saved methods page)
function scrollPaymentOptions(direction) {
    const scrollContainer = document.getElementById('paymentOptionsScroll');
...
// Scroll payment options horizontally (saved methods page)
function scrollPaymentOptionsSaved(direction) {
    const scrollContainer = document.getElementById('addPaymentOptionsSaved');
    if (!scrollContainer) return;
    ...
    // Update arrow states after scroll
    setTimeout(() => updateScrollArrowsSaved(), 100);
}
...
// Update scroll arrow visibility for saved methods page
function updateScrollArrowsSaved() {
    const scrollContainer = document.getElementById('addPaymentOptionsSaved');
    const leftArrow = document.getElementById('scrollLeftBtnSaved');
    const rightArrow = document.getElementById('scrollRightBtnSaved');
...
function addPaymentMethod(type) {
    ...
    // Check if we're on the "no saved methods" page (horizontal layout or grid layout)
    const horizontalSection = document.getElementById('paymentOptionsHorizontal');
    const gridSection = document.getElementById('paymentOptionsGrid');
    const isNoSavedMethodsPage = !!(horizontalSection || gridSection);
    
    if (isNoSavedMethodsPage) {
        // On no saved methods page, update selected state
        const selector = horizontalSection ? '.payment-options-scroll .add-payment-option' : '.payment-options-grid .add-payment-option';
        const allOptions = document.querySelectorAll(selector);
        allOptions.forEach(opt => opt.classList.remove('selected'));
        
        // Find and select the clicked option
        const clickedOption = event.target.closest('.add-payment-option');
        if (clickedOption) {
            clickedOption.classList.add('selected');
        }
    } else {
        // On saved methods page, update selected state within add payment section
        const allOptions = document.querySelectorAll('#addPaymentOptionsSaved .add-payment-option');
        allOptions.forEach(opt => opt.classList.remove('selected'));
    ...
    // Handle based on type - force open on saved methods page
    const forceOpen = !isNoSavedMethodsPage;
    
    switch(type) {
        case 'credit':
            toggleNewCardForm('credit', forceOpen);
            break;
        case 'debit':
            toggleNewCardForm('debit', forceOpen);
            break;
        case 'card':
            toggleNewCardForm('credit', forceOpen);
            break;
        case 'bank':
            toggleNewBankForm(forceOpen);
            break;
        case 'link':
            showPlaceholder('Link', 'Link payment method will be available soon. Please select another payment option to continue.');
            break;
        case 'bancontact':
            showPlaceholder('Bancontact', 'Bancontact payment method will be available soon. Please select another payment option to continue.');
            break;
        case 'sofort':
            showPlaceholder('Sofort', 'Sofort payment method will be available soon. Please select another payment option to continue.');
            break;
        default:
...
function toggleNewCardForm(cardType = 'credit', forceOpen = false) {
    ...
    // On no saved methods page, always show the form (just switch content)
    if (isNoSavedMethodsPage) {
        // Hide bank form and placeholder if they're open
        if (bankSection) {
            bankSection.classList.remove('active');
        }
        const placeholderSection = document.getElementById('placeholderSection');
        if (placeholderSection) {
            placeholderSection.classList.remove('active');
        }
        
        // Always show card form
        ...
        // Scroll to form if not already visible
        setTimeout(() => {
    ...
    // Original toggle logic for saved methods page
    if (isActive && !forceOpen) {
        ...
        // Hide bank form and placeholder when showing card form
        if (bankSection) {
            bankSection.classList.remove('active');
        }
        const placeholderSection = document.getElementById('placeholderSection');
        ...
        // Hide Add Payment button when form is open (only if not in add payment section)
        const addPaymentSection = document.getElementById('addPaymentSection');
        if (addPaymentBtn && (!addPaymentSection || !addPaymentSection.classList.contains('active'))) {
...
// Show placeholder for unavailable payment methods
function showPlaceholder(methodName, message) {
    const placeholderSection = document.getElementById('placeholderSection');
    const cardSection = document.getElementById('newCardSection');
    const bankSection = document.getElementById('newBankSection');
    const titleElement = document.getElementById('placeholderTitle');
    const textElement = document.getElementById('placeholderText');
    ...
    if (titleElement) {
        titleElement.textContent = `${methodName} - Coming Soon`;
    }
    if (textElement) {
    ...
    if (placeholderSection) {
        ...
        setTimeout(() => {
            placeholderSection.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }, 100);
    }
}

// Toggle new bank account form
function toggleNewBankForm(forceOpen = false) {
    ...
    // On no saved methods page, always show the form
    if (isNoSavedMethodsPage) {
        // Hide card form and placeholder if they're open
        if (cardSection) {
            cardSection.classList.remove('active');
        }
        const placeholderSection = document.getElementById('placeholderSection');
    ...
    if (isActive && !forceOpen) {
        ...
        // Hide card form and placeholder when showing bank form
        if (cardSection) {
            cardSection.classList.remove('active');
        }
        const placeholderSection = document.getElementById('placeholderSection');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize scroll arrows for horizontal payment options (no saved methods page)
    const scrollContainer = document.getElementById('paymentOptionsScroll');
    if (scrollContainer) {
        // Initial arrow state
        updateScrollArrows();
    ...
    // Initialize scroll arrows for saved methods page (when add payment section opens)
    const addPaymentSection = document.getElementById('addPaymentSection');
    if (addPaymentSection) {
        // Watch for when the section becomes active
        const observer = new MutationObserver(() => {
            if (addPaymentSection.classList.contains('active')) {
                setTimeout(() => {
                    updateScrollArrowsSaved();
                    const scrollContainerSaved = document.getElementById('addPaymentOptionsSaved');
                    if (scrollContainerSaved) {
                        scrollContainerSaved.addEventListener('scroll', () => {
                            updateScrollArrowsSaved();
                        });
                    }
                }, 100);
        ...
        observer.observe(addPaymentSection, { attributes: true, attributeFilter: ['class'] });
    }
});
